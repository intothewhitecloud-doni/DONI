...
const fixedCanvasPromptAliases: Record<FixedScenarioId, string[]> = {
  "customer-a-claims": ["고객A 클레임 원인", "고객 A 클레임 원인", "고객A 보상 요청", "고객A 클레임 분석"],
...
export function getAiCanvasOverview(state: PrototypeState): AiCanvasOverview {
  ...
  return {
    ...
    recommendations: [
      recommendation("overview-risk", "위험 신호 우선순위 확인", "마진, 클레임, 처리 시간을 같은 우선순위 표로 먼저 정렬합니다.", "분석 순서 명확화", "danger"),
  ...
  switch (scenarioId) {
      ...
      return {
        ...
        visualBlocks: [
          ...metricBlocksFor(state, [
            ...
            ["metric-claim-rate", "visual-canvas-risk-claims", "고객A 클레임률"],
            ["metric-delay-time", "visual-canvas-risk-delay", "평균 주문 처리 시간"]
          ]),
          comparisonVisualBlock("visual-highest-risk-priority", "위험 신호 우선순위", [
            ...
            { label: "2순위", value: "고객A 클레임", detail: metricDeltaDetail(state, "metric-claim-rate"), tone: "danger" },
        ...
        recommendations: [
          ...
          recommendation("risk-customer", "고객A 선제 안내", "배송 지연과 보상 요청을 먼저 정리해 이탈 위험을 낮춥니다.", "클레임 재발 감소", "warning", insightAction("고객A 인사이트 보기", "insight-customer-claims")),
          recommendation("risk-supplier", "공급업체 A사 조건 재협의", "납품 준수율과 주문 처리 시간을 기준으로 개선 조건을 협의합니다.", "처리 시간 단축", "info", objectAction("공급업체 A사 보기", "entity-supplier-a"))
        ],
        actionItems: [
    ...
    case "p42-margin-cause":
      return {
        ...
        visualBlocks: [
          ...
          ...metricBlocksFor(state, [
            ["metric-delay-time", "visual-canvas-margin-delay-time", "주문 처리 시간"],
      ...
      return {
        ...
        visualBlocks: [
          ...
          comparisonVisualBlock("visual-customer-a-impact", "고객A 영향 요약", [
            ...
            { label: "주문 처리 시간", value: formatMetric(state, "metric-delay-time"), detail: metricDeltaDetail(state, "metric-delay-time"), tone: "warning" }
          ]),
          ...metricBlocksFor(state, [
            ["metric-delay-time", "visual-canvas-claims-delay-time", "주문 처리 시간"],
        ...
        recommendations: [
          recommendation("claim-notice", "고객A 선제 안내", "지연 사유와 보상 기준을 먼저 안내합니다.", "이탈 위험 완화", "danger", insightAction("고객A 인사이트 보기", "insight-customer-claims")),
          ...
          recommendation("claim-monitor", "2주 클레임 추세 모니터링", "클레임률과 처리 시간을 같은 기간으로 추적합니다.", "재발 신호 조기 탐지", "info", objectAction("고객A 관리 대상 보기", "entity-customer-core"))
        ],
        actionItems: [
    ...
    default:
      return {
        ...
        shortLabel: scenario.shortLabel,
        title: "공급업체 A사 영향 분석",
        subtitle: "납품 준수율과 주문 처리 시간을 기반으로 P-42 공급 리스크를 canvas로 요약합니다.",
        summary: [
          "공급업체 A사는 현재 P-42 제품의 출고 지연과 생산 병목에 직접적인 영향을 주는 핵심 공급 리스크로 판단됩니다.",
          `최근 데이터 기준으로 A사와 연결된 P-42의 납품 준수율은 70~72% 수준으로 낮고, 평균 주문 처리 시간은 ${formatMetric(state, "metric-delay-time")}까지 증가했습니다.`,
        ...
        visualBlocks: [
          ...
          metricPreviousCurrentBlock(state, "metric-delay-time", "visual-supplier-a-delay-time", "평균 주문 처리 시간", "이전 26시간에서 현재 36.8시간으로 늘어난 처리 시간 증가를 강조합니다."),
          ...metricBlocksFor(state, [
        ...
        keyFindings: [
          ...
          "납품 준수율은 70~72% 수준이고 평균 주문 처리 시간은 36.8시간까지 증가했습니다.",
        ...
        recommendations: [
          ...
          recommendation("supplier-watch", "2주간 지표 모니터링", "납품 준수율과 주문 처리 시간을 같은 주기로 추적합니다.", "개선 여부 확인", "info", metricAction("주문 처리 시간 보기", "metric-delay-time"))
        ],
        actionItems: [
...
function riskPriorityTable(state: PrototypeState): AiCanvasDetailTable {
  return {
    ...
    rows: [
      ...
      { id: "risk-2", cells: ["2", "고객A 클레임", formatMetric(state, "metric-claim-rate"), "반복 보상 요청과 이탈 위험 확대"], tone: "danger" },
      { id: "risk-3", cells: ["3", "공급업체 A사", formatMetric(state, "metric-delay-time"), "납품 준수율과 처리 시간 악화"], tone: "warning" }
    ]
  };
}
...
function supplierImpactTable(state: PrototypeState): AiCanvasDetailTable {
  return {
    ...
    rows: [
      ...
      { id: "supplier-delay", cells: ["주문 처리 시간", formatMetric(state, "metric-delay-time"), "긴급 대응 비용 증가", "2주 모니터링"], tone: "danger" },
...
function customerClaimsTable(state: PrototypeState): AiCanvasDetailTable {
  return {
    title: "고객A 영향 상세",
    caption: "클레임률과 주문 처리 시간의 동시 악화 흐름입니다.",
    columns: ["항목", "현재 값", "영향", "권장 대응"],
    rows: [
      { id: "claims-rate", cells: ["클레임률", formatMetric(state, "metric-claim-rate"), "보상 요청 반복", "선제 안내"], tone: "danger" },
      { id: "claims-delay", cells: ["주문 처리 시간", formatMetric(state, "metric-delay-time"), "배송 지연 체감", "납기 사유 공유"], tone: "warning" },
      { id: "claims-customer", cells: ["반복 구매 고객군", "고객A", "이탈 위험 증가", "보상 기준 조정"], tone: "warning" }
    ]
  };
}
